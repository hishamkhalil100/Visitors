...
namespace Vistors
{
    public partial class Vistors_VistorTracking : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dlUsers.DataSource = new VisitorsDataContext().VistorTacking(int.Parse(Request["ID"].ToString()), endDate, startDate).ToList<VistorTackingResult>();
                dlUsers.DataBind();
            }
            catch (Exception ex)
            {
        ...
        private void getData()
        {
            ...
            var q = obj.VistorsSearch(null, null, null, null, int.Parse(Request["ID"].ToString())).Single<VistorsSearchResult>();
            lblName.Text = q.Name;
            lblMobile.Text = q.MobileNo;
            lblNID.Text = q.CivilId;
            //lblCountry.Text = obj.CountriesSearch(q.CountryID).Single<CountriesSearchResult>().CountryName;
            // hrefProfileImg.HRef = ConfigurationManager.AppSettings["image"] + q.NationalIDImg;
            //imgProfile.ImageUrl = ConfigurationManager.AppSettings["image"] + q.NationalIDImg;
            if (q.Gender == true)
            {
                lblGender.Text = "ذكر";
            }
            ...
            lnlNIDType.Text = "هوية وطنية / هوية مقيم";


            dlUsers.DataSource = obj.VistorTacking(int.Parse(Request["ID"].ToString()), null, null).ToList<VistorTackingResult>();
            dlUsers.DataBind();
        }
